...
async function handleClerkWebhook(req: NextRequest): Promise<NextResponse> {
  ...
  if (!webhookSecret) {
    logger.error({ action: 'webhook_secret_missing' }, 'CLERK_WEBHOOK_SECRET not configured')
  ...
  if (!svixId || !svixTimestamp || !svixSignature) {
    logger.warn({
      ...
      headers: { svixId: !!svixId, svixTimestamp: !!svixTimestamp, svixSignature: !!svixSignature }
  ...
  try {
    ...
    logger.error({ error: error instanceof Error ? error.message : 'Unknown error' }, 'Failed to parse webhook payload')
  ...
  try {
    ...
    logger.info({ action: 'webhook_signature_verified' }, 'Webhook signature verified')
  } catch (error) {
    logger.error({
